...
function submitFormResponse() {
    ...
    questionToAnswerMap[DAMAGE] = JSON.stringify(_damage);
...
function inflateDamageDiv() {
    var question = _form.questions[DAMAGE];

    var questionDiv = document.getElementById("damageDiv");
    ...
    for (i = 0; i < question.options.length; i++) {
        ...
        options.onclick = function () {
            _damage = this.value;
            document.getElementById(_damage).checked = true;
            invalidateFooter();
        };
...
function inflateSummaryView() {
    ...
    var damageDetailsDiv = KASClient.UI.getElement("div", divAttributes);

    var damageDetailsHeader = KASClient.UI.getElement("div", {
        "padding": "14px",
        "padding-bottom": "0pt"
    });

    damageDetailsHeader.className = "comment-header";
    damageDetailsHeader.innerText = _strings[_form.questions[DAMAGE].title];
    KASClient.UI.addElement(damageDetailsHeader, damageDetailsDiv);

    var damageDetailsView = KASClient.UI.getElement("div", {
        "padding": "14px",
        "padding-top": "5pt"
    });

    var damage = KASClient.UI.getElement("div", {
        "color": "#32485f",
        "font-size": "12pt",
        "overflow-wrap": "break-word",
        "word-wrap": "break-word",
    ...
    damage.innerHTML = _strings[_form.questions[DAMAGE].options[_damage].text];

    KASClient.UI.addElement(damage, damageDetailsView);
    KASClient.UI.addElement(damageDetailsView, damageDetailsDiv);
    ...
    KASClient.UI.addElement(damageDetailsDiv, summaryView);
