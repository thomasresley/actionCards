...
function submitFormResponse() {
    ...
    questionToAnswerMap[TYPE] = _type;
...
function inflateTypeDiv() {
    var question = _form.questions[TYPE];

    var questionDiv = document.getElementById("typeDiv");
    ...
    for (i = 0; i < question.options.length; i++) {
        ...
        options.onclick = function () {
            _type = this.value;
            document.getElementById(_type).checked = true;
            invalidateFooter();
        };
...
function inflateSummaryView() {
    ...
    var typeDetailsDiv = KASClient.UI.getElement("div", divAttributes);

    var typeDetailsHeader = KASClient.UI.getElement("div", {
        "padding": "14px",
        "padding-bottom": "0pt"
    });

    typeDetailsHeader.className = "comment-header";
    typeDetailsHeader.innerText = _strings[_form.questions[TYPE].title];
    KASClient.UI.addElement(typeDetailsHeader, typeDetailsDiv);

    var typeDetailsView = KASClient.UI.getElement("div", {
        "padding": "14px",
        "padding-top": "5pt"
    });

    var type = KASClient.UI.getElement("div", {
        "color": "#32485f",
        "font-size": "12pt",
        "overflow-wrap": "break-word",
        "word-wrap": "break-word",
    ...
    type.innerHTML = _strings[_form.questions[TYPE].options[_type].text];

    KASClient.UI.addElement(type, typeDetailsView);
    KASClient.UI.addElement(typeDetailsView, typeDetailsDiv);
    ...
    KASClient.UI.addElement(typeDetailsDiv, summaryView);
