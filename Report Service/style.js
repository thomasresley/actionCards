...
function submitFormResponse() {
    ...
    questionToAnswerMap[PARTSNEEDED] = _partsNeeded;
...
function inflateDueDateDiv() {
    ...
    KASClient.UI.clearElement(dueDateDiv);

    var dueDateTitle = KASClient.UI.getElement("div");
    dueDateTitle.className = "question-title";
    dueDateTitle.innerText = _strings[_form.questions[DUEDATE].title];

    var dueDateInput = KASClient.UI.getElement("input");
    dueDateInput.id = "dueDate";
    dueDateInput.type = "text";
    dueDateInput.className = "comment-input";

    if (KASClient.getPlatform() == KASClient.Platform.iOS) {
        KASClient.UI.addCSS(dueDateInput, {
            "padding-left": "13pt"
        });
    }
    dueDateInput.placeholder = _strings["strDateFormat"];
    dueDateInput.addEventListener("input", function () {
        _dueDate = this.value;
        invalidateFooter();
    });

    KASClient.UI.addElement(dueDateTitle, dueDateDiv);
    KASClient.UI.addElement(dueDateInput, dueDateDiv);
}
...
function inflateGarageDiv() {
    ...
    for (i = 0; i < question.options.length; i++) {
        ...
        options.onclick = function () {
            _garage = this.value;
            document.getElementById(_garage).checked = true;
            invalidateFooter();
        };
...
function inflatePartsNeededDiv() {
    var partsNeededDiv = document.getElementById("partsNeededDiv");
    KASClient.UI.clearElement(partsNeededDiv);

    var partsNeededTitle = KASClient.UI.getElement("div");
    partsNeededTitle.className = "question-title";
    partsNeededTitle.innerText = _strings[_form.questions[PARTSNEEDED].title];

    var partsNeededInput = KASClient.UI.getElement("input");
    partsNeededInput.type = "text";
    partsNeededInput.className = "comment-input";

    if (KASClient.getPlatform() == KASClient.Platform.iOS) {
        KASClient.UI.addCSS(partsNeededInput, {
            "padding-left": "13pt"
        });
    }
    partsNeededInput.placeholder = _strings["strPartsNeeded"];
    partsNeededInput.addEventListener("input", function (event) {
        _partsNeeded = event.target.value;
        invalidateFooter();
    });

    KASClient.UI.addElement(partsNeededTitle, partsNeededDiv);
    KASClient.UI.addElement(partsNeededInput, partsNeededDiv);
}
...
function inflateFooterView() {
    ...
    if (_currentPage == 1) {
        ...
        if (_vehicleReg == "" || _partsNeeded == "" || _odometer == "" || _garage == -1) {
...
function inflateSummaryView() {
    ...
    var garageDetailsDiv = KASClient.UI.getElement("div", divAttributes);

    var garageDetailsHeader = KASClient.UI.getElement("div", {
        "padding": "14px",
        "padding-bottom": "0pt"
    });

    garageDetailsHeader.className = "comment-header";
    garageDetailsHeader.innerText = _strings[_form.questions[GARAGE].title];
    KASClient.UI.addElement(garageDetailsHeader, garageDetailsDiv);

    var garageDetailsView = KASClient.UI.getElement("div", {
        "padding": "14px",
        "padding-top": "5pt"
    });

    var garage = KASClient.UI.getElement("div", {
        "color": "#32485f",
        "font-size": "12pt",
        "overflow-wrap": "break-word",
        "word-wrap": "break-word",
    ...
    garage.innerHTML = _strings[_form.questions[GARAGE].options[_garage].text];

    KASClient.UI.addElement(garage, garageDetailsView);
    KASClient.UI.addElement(garageDetailsView, garageDetailsDiv);

    // partsNeeded

    var partsNeededDetailsDiv = KASClient.UI.getElement("div", divAttributes);

    var partsNeededDetailsHeader = KASClient.UI.getElement("div", {
        "padding": "14px",
        "padding-bottom": "0pt"
    });

    partsNeededDetailsHeader.className = "comment-header";
    partsNeededDetailsHeader.innerText = _strings[_form.questions[PARTSNEEDED].title];
    KASClient.UI.addElement(partsNeededDetailsHeader, partsNeededDetailsDiv);

    var partsNeededDetailsView = KASClient.UI.getElement("div", {
        "padding": "14px",
        "padding-top": "5pt"
    });

    var partsNeeded = KASClient.UI.getElement("div", {
        "color": "#32485f",
        "font-size": "12pt",
        "overflow-wrap": "break-word",
        "word-wrap": "break-word",
    ...
    partsNeeded.innerHTML = _partsNeeded;

    KASClient.UI.addElement(partsNeeded, partsNeededDetailsView);
    KASClient.UI.addElement(partsNeededDetailsView, partsNeededDetailsDiv);
    ...
    KASClient.UI.addElement(garageDetailsDiv, summaryView);
    KASClient.UI.addElement(partsNeededDetailsDiv, summaryView);
