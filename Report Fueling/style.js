...
function inflateFuelStationDiv() {
    ...
    for (i = 0; i < question.options.length; i++) {
        ...
        options.onclick = function () {
            ...
            document.getElementById(_fuelStation).checked = true;
            invalidateFooter();
        };
...
function inflateCostDiv() {
    ...
    KASClient.UI.clearElement(costDiv);

    var costTitle = KASClient.UI.getElement("div");
    costTitle.className = "question-title";
    costTitle.innerText = _strings[_form.questions[COST].title];

    var costInput = KASClient.UI.getElement("input");
    costInput.type = "number";
    costInput.className = "comment-input";

    if (KASClient.getPlatform() == KASClient.Platform.iOS) {
        KASClient.UI.addCSS(costInput, {
            "padding-left": "13pt"
        });
    }
    costInput.placeholder = _strings["strTapToEnter"];
    costInput.addEventListener("input", function (event) {
        _cost = event.target.value;
        invalidateFooter();
    });

    KASClient.UI.addElement(costTitle, costDiv);
    KASClient.UI.addElement(costInput, costDiv);
}
...
function inflateFooterView() {
    ...
    if (_currentPage == 1) {
        ...
        if (_vehicleReg == "" || _fuelStation == -1 || _cost == "" || _litres == "") {
