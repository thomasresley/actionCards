                        ...
                        KASClient.App.getUsersDetailsAsync([userId], function (users, error) {
                            ...
                            xhr.onload = function (e) {
                                if (xhr.status === 200) {
                                    ...
                                    _isTripActive = false;
                                    showError("You don't have an active trip.");
                                }
                            };
                            xhr.send(null);
        ...
        function submitFormResponse() {
            ...
            questionToAnswerMap[ODOMETERLATER] = _odometerLater;
        ...
        function inflateOdometerLaterDiv() {
            var odometerLaterDiv = document.getElementById("odometerLaterDiv");
            KASClient.UI.clearElement(odometerLaterDiv);

            var odometerLaterTitle = KASClient.UI.getElement("div");
            odometerLaterTitle.className = "question-title";
            odometerLaterTitle.innerText = _strings[_form.questions[ODOMETERLATER].title];

            var odometerLaterInput = KASClient.UI.getElement("input");
            odometerLaterInput.type = "text";
            odometerLaterInput.className = "comment-input";

            if (KASClient.getPlatform() == KASClient.Platform.iOS) {
                KASClient.UI.addCSS(odometerLaterInput, {
                    "padding-left": "13pt"
                });
            }
            odometerLaterInput.placeholder = _strings["strTapToEnter"];
            odometerLaterInput.addEventListener("input", function (event) {
                _odometerLater = event.target.value;
                invalidateFooter();
            });

            KASClient.UI.addElement(odometerLaterTitle, odometerLaterDiv);
            KASClient.UI.addElement(odometerLaterInput, odometerLaterDiv);
        }

        function inflateDestinationDiv() {
            var destinationDiv = document.getElementById("destinationDiv");
            KASClient.UI.clearElement(destinationDiv);

            var destinationTitle = KASClient.UI.getElement("div");
            destinationTitle.className = "question-title";
            destinationTitle.innerText = _strings[_form.questions[DESTINATION].title];

            var destinationInput = KASClient.UI.getElement("input");
            destinationInput.type = "text";
            destinationInput.className = "comment-input";

            if (KASClient.getPlatform() == KASClient.Platform.iOS) {
                KASClient.UI.addCSS(destinationInput, {
                    "padding-left": "13pt"
                });
            }
            destinationInput.placeholder = _strings["strTapToEnter"];
            destinationInput.addEventListener("input", function (event) {
                _destination = event.target.value;
                invalidateFooter();
            });

            KASClient.UI.addElement(destinationTitle, destinationDiv);
            KASClient.UI.addElement(destinationInput, destinationDiv);
        }

        function inflateQuestions() {

            inflateOdometerLaterDiv();
        ...
        function inflateFooterView() {
            ...
            if (_currentPage == 1) {
                ...
                if (_odometerLater == "" || _destination == "") {
        ...
        function inflateSummaryView() {
            ...
            var odometerLaterDetailsDiv = KASClient.UI.getElement("div", divAttributes);

            var odometerLaterDetailsHeader = KASClient.UI.getElement("div", {
                "padding": "14px",
                "padding-bottom": "0pt"
            });

            odometerLaterDetailsHeader.className = "comment-header";
            odometerLaterDetailsHeader.innerText = _strings[_form.questions[ODOMETERLATER].title];
            KASClient.UI.addElement(odometerLaterDetailsHeader, odometerLaterDetailsDiv);

            var odometerLaterDetailsView = KASClient.UI.getElement("div", {
                "padding": "14px",
                "padding-top": "5pt"
            });

            var odometerLater = KASClient.UI.getElement("div", {
                "color": "#32485f",
                "font-size": "12pt",
            ...
            odometerLater.innerHTML = _odometerLater;

            KASClient.UI.addElement(odometerLater, odometerLaterDetailsView);
            KASClient.UI.addElement(odometerLaterDetailsView, odometerLaterDetailsDiv);

            // destination

            var destinationDetailsDiv = KASClient.UI.getElement("div", divAttributes);

            var destinationDetailsHeader = KASClient.UI.getElement("div", {
                "padding": "14px",
                "padding-bottom": "0pt"
            });

            destinationDetailsHeader.className = "comment-header";
            destinationDetailsHeader.innerText = _strings[_form.questions[DESTINATION].title];
            KASClient.UI.addElement(destinationDetailsHeader, destinationDetailsDiv);

            var destinationDetailsView = KASClient.UI.getElement("div", {
                "padding": "14px",
                "padding-top": "5pt"
            });

            var destination = KASClient.UI.getElement("div", {
                "color": "#32485f",
                "font-size": "12pt",
                "overflow-wrap": "break-word",
                "word-wrap": "break-word",
            ...
            destination.innerHTML = _destination;

            KASClient.UI.addElement(destination, destinationDetailsView);
            KASClient.UI.addElement(destinationDetailsView, destinationDetailsDiv);
            ...
            KASClient.UI.addElement(odometerLaterDetailsDiv, summaryView);
            KASClient.UI.addElement(destinationDetailsDiv, summaryView);
